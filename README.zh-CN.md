...
| `claude-deep …` | 走，**锁定 deepseek 家族** | GUI 选中的必须是 `deepseek-*` 卡片；不是的话直接 400 报错告诉你怎么处理 |
| `claude-3 …` | 走，不限家族 | 用路由面板里**选中**那张卡片对应的 profile，BuerAI / 自定义都行 |
| `OI_PROFILE=<id> claude-3 …`（高级） | 走 | 这一次调用临时换一个 profile（不影响 GUI 选择，也不影响别的终端）。`claude-deep` 也支持同样的 env override |

**命令名即后端**：三个命令对应三种意图——Anthropic 用 `claude`，
DeepSeek 用 `claude-deep`，其他（BuerAI / 自定义）用 `claude-3`。
不需要每次去 GUI 切卡片或者背 env var。`claude-deep` 跑的时候 GUI
不是 deepseek 卡的话，代理会返回结构化 400 提示你切卡或换 `claude-3`。
...
**为啥 `claude` 必须留独立**：Anthropic Max / Pro 的 OAuth token 走任何
代理都失败（端到端客户端身份校验），所以原版二进制必须保留为不被
劫持的直连通道，所有走代理 + 路由的逻辑归 `claude-deep` / `claude-3`。

**多终端并行**：所有 shim 默认读 GUI active；`OI_PROFILE` 是 per-call
覆盖。所以终端 A 一直挂 BuerAI Pro 默认，终端 B 跑脚本时随手 `OI_PROFILE=deepseek-v4-flash claude-3`，互不干扰。

**安装**：上述 shim（`claude-3` / `claude-deep` / `claude-native` / `oi-claude`）
由 Open Island 启动时自动同步到 `~/.open-island/bin/`。把这个目录加到 PATH
（`path+=("$HOME/.open-island/bin")`）即可。详细步骤见
