...
**Open Island 是 Vibe Island 的开源替代品吗？**
是的。Open Island 是 [Vibe Island](https://vibeisland.app/) 的免费、开源（GPL v3）、本地优先的替代品——可 fork、可改、可以发布你自己的版本。
...
**支持哪些 AI coding agent？**
9 个：Claude Code、Codex、Cursor、Gemini CLI、OpenCode、Qoder、Qwen Code、Factory、CodeBuddy——一个界面全部覆盖。

**没有刘海的 Mac 能用吗？**
能。没有刘海的 Mac 和外接显示器上会自动降级为紧凑的顶部居中栏。

**会把我的数据发到云端吗？**
不会。本地优先——无服务器、无遥测、无需账号，一切在你的 Mac 上运行。

**支持 tmux / iTerm2 / Ghostty / VS Code / Cursor / Warp 吗？**
支持。15+ 终端和 IDE：终端类（Terminal.app、Ghostty、iTerm2、tmux、WezTerm、Zellij、cmux、Kaku、Warp）完整 jump-back；IDE 类（VS Code、Cursor、Windsurf、Trae、JetBrains 全家桶）工作区级跳转。

**和 Claude Code / Codex / Cursor 怎么集成？**
通过每个 agent 原生的 hook 系统（如 `~/.claude/settings.json`、`~/.codex/config.toml`、`~/.cursor/hooks.json`）。安装/卸载在 app 内管理。Hooks **fail open**——Open Island 没运行时不影响 agent 继续工作。

**Open Island 会读取我的源代码吗？**
不会。只接收 hook 事件（session 开始、prompt 提交、权限请求、stop 等），不会读取你的项目文件。

**怎么贡献代码？**
项目所有代码都由 AI 产出。把 [CONTRIBUTING.zh-CN.md](CONTRIBUTING.zh-CN.md) 里的 prompt 复制到你的 coding agent，就可以开始迭代。
